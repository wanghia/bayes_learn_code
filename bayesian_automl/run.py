...
default_program_dir = root_dir + "jungtaek-kim"
...
if __name__=="__main__" and debug_mode < 4:
    ...
    for basename in datanames: # Loop over datasets
        ...
        is_dim = False
        if n_components > num_train or n_components > num_valid or n_components > num_test:
            n_components = min(num_train, 
                num_valid, 
                num_test)
        ...
        if is_dim:
            pca = sklearn.decomposition.KernelPCA(n_components=n_components, kernel='rbf')
            D.data['X_train'] = pca.fit_transform(D.data['X_train'])
            D.data['X_valid'] = pca.fit_transform(D.data['X_valid'])
            D.data['X_test'] = pca.fit_transform(D.data['X_test'])
            D.info['feat_num'] = n_components
...
#                if new_n_estimators <= n_estimators: 
#                    break
...
#                M.model.n_estimators += int((M.model.n_estimators + 1))
